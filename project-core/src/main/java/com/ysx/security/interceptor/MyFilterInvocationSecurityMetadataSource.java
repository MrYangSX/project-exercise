package com.ysx.security.interceptor;
...
public class MyFilterInvocationSecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
    ...
    public Collection<ConfigAttribute> getAttributes(Object o) {
        ...
        //去数据库查询资源
        List<SysPermission> permissionList = sysPermissionService.findPermissions();
        for (SysPermission permission : permissionList) {
        	//权限id
        	String pId = permission.getId();
            if (antPathMatcher.match(permission.getUrl(), requestUrl)){
            	
            	List<SysRole> sysRoles = sysRoleService.findSysRolesByPId(pId);
            	String[] values = new String[sysRoles.size()];
            	if (sysRoles.size() > 0) {
            		for (int i = 0; i < sysRoles.size(); i++) {
                        values[i] = sysRoles.get(i).getRoleNameEn();
                    }
				}
